...
type Periodo = "todo" | "dia" | "mes" | "anio";
...
export function ImprimirInformeAdmin({ fuente, comunas }: { fuente: FilaAdmin[]; comunas: string[] }) {
  ...
  async function imprimir(todo: boolean) {
    ...
    const etiquetaP = { todo: "todo el período", dia: "hoy", mes: "este mes", anio: "este año" }[periodo];
    const filtros = todo ? "Toda la información" : `Período: ${etiquetaP} · Edad: ${edad} · Sexo: ${sexo} · Comuna: ${comuna}`;
    try {
      await pdfAdmin(datos, filtros, [
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-1"><Label htmlFor="p-periodo">Rango de fechas</Label>
            <select id="p-periodo" className={selectCls} value={periodo} onChange={(e) => setPeriodo(e.target.value as Periodo)}>
              <option value="todo">Todo</option><option value="dia">Día (hoy)</option><option value="mes">Mes actual</option><option value="anio">Año actual</option>
            </select></div>
